...
| `filter.js` | `[data-filter-group]` | Button-driven token filter (Buff port). Buttons: `[data-filter-target="token\|all"]`; items: `[data-filter-name="token…"]`. JS sets `data-filter-status="active\|not-active\|transition-out"` on items + buttons — style/hide via CSS. |
